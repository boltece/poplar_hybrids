...
# # Ongoing gene flow model (GF; isolation-with-migration) with two categories of loci experiencing heterogeneous migration rates across the genome


pts_1 = [250,260,270]

def GFhet(params, ns, pts):
	nuB, nuT, T1, m12, m21, m12h, m21h, P = params
	"""
	2-populations, strict isolation model with no gene flow.

	nuB:  Current size of balsamifera population, after split.
	...
	phiI = dadi.Integration.two_pops(phiI,xx,T1, nu1=nuB, nu2=nuT, m12=m12h,m21=m21h)
	fsI = dadi.Spectrum.from_phi(phiI,ns,(xx,xx))
	
	### Sum the two spectra in proportions P and 1-P, *without* integrating the effect of misorientation of ancestor (polarized vcf with consensus ancestral state)
	fs = P*fsUn + (1-P)*fsI
	return fs
...
upper_bound = [10, 10, 10, 100, 100, 100, 100, 0.9]
lower_bound = [1e-4,1e-4, 1e-4, 1e-3, 1e-3, 1e-3, 1e-3, 0.01]
p0 = array([0.1,0.1, 0.1, 1, 1, 1, 1,0.4])
...
matplotlib.pyplot.savefig("GFhet_unfolded.pdf")
