...
# # Secondary contact model (SC) with pop size change (PSC) with two categories of loci experiencing heterogeneous migration rates throughout the genome


pts_1 = [250,260,270]

def PSCSChet(params, ns, pts):
	...
	### Sum the two spectra in proportions P and 1-P, *without* integrating the effect of misorientation of ancestor (ancestral state was built from consensus)
	fs = P*fsUn + (1-P)*fsI
	return fs
...
upper_bound = [10, 10,0.1, 10, 10, 0.1, 100, 100, 100, 100, 0.9]
lower_bound = [1e-5,1e-5, 1e-7,1e-5,1e-5, 1e-7, 1e-3, 1e-3, 1e-3, 1e-3, 0.01]
p0 = array([0.01,0.01, 0.001, 0.01, 0.01, 0.001, 5, 5, 5, 5,0.4])
...
matplotlib.pyplot.savefig("PSCSChet_unfolded.pdf")
